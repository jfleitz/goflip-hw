...
struct message_struct* AddToQueueWithPriority(struct message_struct **queue, message_type messageType, int commandID, int commandValue, int priority) {
	...
	printf("AddToQueueWithPriority:init\n");
	...
	printf("AddToQueueWithPriority:Setting params\n");
	...
	if(seekptr==NULL){
		...
		*queue = ptr;
		printf("AddToQueueWithPriority:No messages. Setting Queue to new message\n");
		return ptr;
	}

	printf("AddToQueueWithPriority:while loop\n");
	
	//check to see if we need to add this to the beginning of the list.
	if(seekptr->priority > priority){
		printf("AddToQueueWithPriority:messages. Adding to first of list since priority is highest\n");
		ptr->next = seekptr;
		*queue = ptr;
		return ptr;
	}
	...
	while (seekptr != NULL) {
		...
		{
			printf("AddToQueueWithPriority:whileloop-found after first\n");
			...
			printf("AddToQueueWithPriority:looking for next\n");
	...
	printf("AddToQueueWithPriority:end while\n");

	if (found == 0) { //insert location not found, so add to the end.
		printf("AddToQueueWithPriority:adding to last\n");
